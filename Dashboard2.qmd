...
format:
  dashboard:
    ...
    nav-buttons:
      ...
      - icon: instagram
        href: https://www.instagram.com/hadleywickham/
server: shiny
execute:
  echo: false
  warning: false
...
```{r}
shiny::tags$img(
  src   = base64enc::dataURI(file = "gdp_growth_animation.gif", mime = "image/gif"),
  width = "100%"
)
```
...
```{r leaflet-map}
#| title: "GDP per Capita (PPP) by Country"
...
leafletOutput("gdp_map", height = "450px")
```
...
# I had to readd the master set and the libraries beacues the map wouldn't 
# show up if i didn't. 
...
output$gdp_map <- renderLeaflet({
  ...
  map_data <- masterSet |>
    ...
    mutate(
      is_democracy    = as.character(is_democracy),
      democracy_label = case_when(
  ...
  map_joined <- world |>
    left_join(map_data, by = c("iso_a3" = "country_code"))

  pal <- colorNumeric(
    palette  = "YlOrRd",
    domain   = map_joined$gdp_per_capita_ppp,
    na.color = "#cccccc"
  )

  leaflet(map_joined,
          options = leafletOptions(worldCopyJump = TRUE, minZoom = 2)) |>
    addProviderTiles(providers$CartoDB.Positron) |>
    addPolygons(
      fillColor   = ~pal(gdp_per_capita_ppp),
      fillOpacity = 0.8,
      color       = "white",
      weight      = 0.5,
      label = ~paste0(
        "<b>", name, "</b><br>",
        "GDP per Capita (PPP): $", format(round(gdp_per_capita_ppp), big.mark = ","), "<br>",
        "Regime: ", democracy_label, "<br>",
        "Year: ", selected_year
